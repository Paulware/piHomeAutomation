...
<?php
  ...
  if ($user == "") {
     ...
     print ("<input type=\"button\" value=\"Change Password\" onclick=\"location.href='changePassword.php';\"><br>\n");
     print ("<p>");
     print ("<input type=\"button\" value=\"Add Sensor\" onclick=\"location.href='addSensor.php';\"><br>\n");
     print ("<hr><p><H1>View and control sensors</H1>Note: A water sensor will have a value of 1 when no water is detected and 0 when water is detected.\n" );
  }
...
<?php
     ...
     echo ("<tr><th>MAC</th><th>Value</th><th>Timestamp</th><th>Delete</th><th>Location</th><th>Action</th></tr>\n");
     $id = findUser($user);
     if ($id) {
       $userId = $id['ID'];
       ...
       {		 
         ...
         print ("<tr><td>$MAC</td><td><center>$Value</center></td><td>$Timestamp</td><td><input type=\"button\" value=\"Delete\" onclick=\"deleteSensor('$MAC');\"><td><input name=\"location$count\" value=\"$Location\" onchange=\"modifyNickname('$MAC',this.value);\"></td><td><input onclick=\"changeAction('$MAC');\" type=\"button\" value=\"Modify Action\"></td></tr>");
         $count = $count + 1; 
       }
     }    
   }    
